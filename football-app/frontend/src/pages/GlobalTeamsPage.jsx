...
import { getGlobalTeams, createGlobalTeam, renameGlobalTeam, deleteGlobalTeam } from '../api/footballApi';
...
function TeamCard({ team, onEdit, onDelete, isAdmin }) {
    ...
    <Box>
      ...
      {loading ? (
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 1.5 }}>
          {teams.map((team) => (
            <TeamCard key={team.id} team={team} onEdit={openEdit} onDelete={setDeleteTarget} isAdmin={isAdmin} />
          ))}
        </Box>
      )}
